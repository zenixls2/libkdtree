...
struct _Node : public _Node_base {
  ...
  friend std::basic_ostream<Char, Traits>& operator<<(
      typename std::basic_ostream<Char, Traits>& out, _Node_base const& node);

  template <typename Char, typename Traits>
  friend std::basic_ostream<Char, Traits>& operator<<(
      typename std::basic_ostream<Char, Traits>& out, _Node<_Val> const& node) {
    out << &node;
    out << ' ' << node._M_value;
...
inline typename _Dist::distance_type _S_node_distance(
...
inline const NodeType* _S_node_descend(
    const size_t __dim,
    const _Cmp& __cmp, const _Acc& __acc,
...
template <class SearchVal,
         ...
         typename _Predicate>
inline std::pair<const NodeType*,
  std::pair<size_t, typename _Dist::distance_type>> _S_node_nearest (
    const size_t __k, size_t __dim, SearchVal const& __val,
  ...
  if (near_node && // only visit node's children if node's plane intersect hypersphere
      std::sqrt(_S_node_distance(
          probe_dim % __k, __dist, __acc, __val, probe->_M_value)) <= __max) {
    probe = near_node;
    ++probe_dim;
  }
  while (cur != __end) {
    while (probe != cur) {
      if (_S_node_compare(
            probe_dim % __k, __cmp, __acc, __val, probe->_M_value)) {
        near_node = static_cast<NodePtr>(probe->_M_left);
      ...
      if (pprobe == probe->_M_parent) {// going downward ...
        ...
        if (near_node) {
          probe = near_node;
          ++probe_dim;
        } else if (far_node && // only visit node's children if node's plane intersect hypersphere
        ...
        if (pprobe == near_node && far_node // only visit node's children if node's plane intersect hypersphere
            && std::sqrt(_S_node_distance(probe_dim % __k, __dist, __acc, __val, probe->_M_value)) <= __max) {
          pprobe = probe;
          probe = far_node;
          ++probe_dim;
  ...
  return std::pair<NodePtr, std::pair<size_t, typename _Dist::distance_type>>(
