...
class KDTree : protected _Alloc_base<_Val, _Alloc> {
 ...
 public:
  ...
  KDTree(const KDTree& __x) : _Base(__x.get_allocator()), _M_header(),
      _M_count(0), _M_acc(__x._M_acc), _M_cmp(__x._M_cmp),
      _M_dist(__x._M_dist), __K(__x.__K) {
    _M_empty_initialise();
    // this is slow:
...
/* this does not work since erasure changes sort order
  void erase(const_iterator __A, const_iterator const& __B) {
...
*/
  ...
  std::pair<const_iterator, distance_type> find_nearest(
    ...
    if (_M_get_root()) {
      ...
      best = _S_node_nearest (
          __K, 0, __val, _M_get_root(), &_M_header, _M_get_root(),
 ...
 protected:
  ...
  void _M_check_children(_Link_const_type child, _Link_const_type parent,
  ...
  std::pair<_Link_type,size_type> _M_get_j_min(
      std::pair<_Link_type,size_type> const node, size_type const level) {
  ...
  std::pair<_Link_type,size_type> _M_get_j_max(
      std::pair<_Link_type,size_type> const node, size_type const level) {
    typedef std::pair<_Link_type,size_type> Result;
  ...
  bool _M_matches_node_in_d(_Link_const_type __N, const_reference __V,
  ...
  bool _M_matches_node(_Link_const_type __N, const_reference __V,
    ...
    if (_S_left(__N)) {
      ...
      if (__REGION.intersects_with(__bounds))
        count += _M_count_within_range(
            _S_left(__N), __REGION, __bounds, __L+1);
    }
    if (_S_right(__N)) {
      _Region_ __bounds(__BOUNDS);
  ...
  Visitor _M_visit_within_range(Visitor visitor,
    ...
    if (_S_right(N)) {
      ...
      if (REGION.intersects_with(bounds))
        visitor = _M_visit_within_range(
            visitor, _S_right(N), REGION, bounds, L+1);
    }
  ...
  */
  ...
  friend std::ostream& operator<<(
      std::ostream& o, KDTree<_Val, _Acc, _Dist, _Cmp, _Alloc> const& tree) {
