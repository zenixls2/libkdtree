...
typedef KDTree::KDTree<
  triplet, std::pointer_to_binary_function<triplet,size_t,double>> tree_type;
...
int main() {
...

  {
    ...
    triplet c0(5, 2, 0);
    exact_dist.insert(c0);
    triplet c1(4, 2, 0); exact_dist.insert(c1);
    triplet target(7,4,0);

    std::vector<tree_type::value_type> results;
    exact_dist.find_n_nearest(target, 1, std::back_inserter(results));
    assert(results.size() == size_t(1));
    std::cout << "Test 2 find_n_nearest(), found away from " <<
      target << ", found " << results[0] << std::endl;

    exact_dist.insert({7, 3, 0});
    std::vector<tree_type::value_type> result2;
    exact_dist.find_n_nearest(target, 2, std::back_inserter(result2));
    assert(result2.size() == size_t(2));
    std::cout << "Test 2 find_n_nearest(), found away from " <<
      target << ", found " << result2[0] << ", " << result2[1] << std::endl;
    //assert(results[0].first == std::sqrt(8.0));
